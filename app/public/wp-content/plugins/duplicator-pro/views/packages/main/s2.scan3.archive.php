...
<div class="scan-item">
    ...
    <div class="info">
        ...
        <div id="hb-files-utf8-result" class="dup-tree-section hb-files-style">
            ...
            <span class="input">
                <input type="checkbox" id="filter-names" name="filter-names" onchange="DupPro.Pack.toggleFilterNames()">
                <label for="filter-names">
                    Auto filter files with invalid encoded names
                </label>
            </span>
            <div class="apply-btn">
...
jQuery(document).ready(function($)
{
    ...
    DupPro.Pack.applyFilters = function(btn, type)
    {
        ...
        var dirFilters  = filters.dir;
        var fileFilters = filters.file;
        var nameFilters = filters.names;

        if (dirFilters.length === 0 && fileFilters.length === 0 && nameFilters === false) {
    ...
    DupPro.Pack.initTree = function(tree , data , filterBtn) {
        ...
        treeObj.jstree({
          ...
          "plugins" : [
            ...
            var autoFilterNames = $('#filter-names').is(':checked');
            if (treeObj.jstree("get_selected").length === 0 && !autoFilterNames) {
    ...
    DupPro.Pack.toggleFilterNames = function() {
        var utf8FilterBtn = $("#hb-files-utf8-result .duplicator-pro-quick-filter-btn");
        var filter = DupPro.Pack.getFiltersLists('utf8');

        if (filter.dir.length > 0 || filter.file.length > 0 || filter.names != false) {
            utf8FilterBtn.prop('disabled', false);
        } else {
            utf8FilterBtn.prop('disabled', true);
        }
    }
