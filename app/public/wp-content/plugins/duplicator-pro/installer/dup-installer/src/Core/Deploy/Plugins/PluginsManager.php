    ...
    public function setActions($plugins, $subsiteId = -1)
    {
        ...
        $pluginsList = [
            self::SLUG_SIMPLE_SSL => [
                'longMsg' => $longMsg,
                'info' => 'Really Simple SSL [as Non-SSL installation] will be deactivated',
                'condition' => $isNotSSL,
            ],
            self::SLUG_WP_FORCE_SSL => [
                'longMsg' => $longMsg,
                'info' => 'WP Force SSL & HTTPS Redirect [as Non-SSL installation] will be deactivated',
                'condition' => $isNotSSL,
            ],
            self::SLUG_HTTPS_REDIRECTION => [
                'longMsg' => $longMsg,
                'info' => 'Easy HTTPS Redirection (SSL) [as Non-SSL installation] will be deactivated',
                'condition' => $isNotSSL,
            ],
            self::SLUG_RECAPTCHA => [
                ...
                'info' => 'Simple Google reCAPTCHA [as package creation site URL and the installation site URL are different] will be deactivated',
                'condition' => $paramManager->getValue(PrmMng::PARAM_SITE_URL_OLD) != $paramManager->getValue(PrmMng::PARAM_SITE_URL),
            ]
        ];

        foreach ($pluginsList as $slug => $props) {
            if (isset($this->plugins[$slug]) && $this->plugins[$slug]->isActive($subsiteId) && $props['condition']) {
                Log::info($props['info'], Log::LV_DEBUG);
