        ...
        $params[PrmMng::PARAM_SUBSITE_OVERWRITE_MAPPING] = new ParamFormSitesOwrMap(
            ...
            array(
                ...
                'validateCallback' => function ($value, ParamItem $paramObj) {
                    ...
                    $mainSiteDomain = SnapURL::parseUrl($overwriteData['urls']['home'], PHP_URL_HOST);
                    ...
                    foreach ($value as $map) {
                        switch ($map->getTargetId()) {
                            case SiteOwrMap::NEW_SUBSITE_WITH_SLUG:
                                ...
                                if ($subdomain) {
                                    $newFullUrl = $map->getNewSlug() . '.' . SnapURL::wwwRemove($mainSiteDomain);
                                } else {
                                    $newFullUrl = $mainSiteDomain . '/' . $map->getNewSlug();
                                }
                                break;
