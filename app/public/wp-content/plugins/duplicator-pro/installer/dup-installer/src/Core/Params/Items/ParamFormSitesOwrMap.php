        ...
        for ($i = 0; $i < count($value); $i++) {
            ...
            switch ($targetId) {
                case SiteOwrMap::NEW_SUBSITE_WITH_SLUG:
                    ...
                    break;
                case SiteOwrMap::NEW_SUBSITE_WITH_FULL_DOMAIN:
                    $newSlug = SnapIO::trailingslashit(preg_replace('/[\s"\'\\\\&?#,:;]+/m', '', $newSlug));
                    break;
                default:
                    $newSlug = '';
                    break;
    ...
    protected static function getListExtraData()
    {
        ...
        if (is_null($extraData)) {
            ...
            $extraData['sourceInfo'] = array(
                ...
                'urlScheme'  => self::getUrlScheme($mainSiteUrl),
            ...
            foreach ($archiveConfig->subsites as $subsite) {
                $extraData['sourceInfo']['sites']['id_' . $subsite->id] = array(
                    ...
                    'path' => $subsite->path,
                    'slug' => self::getSubsiteSlug($subsite, $mainSiteUrl, $isSubdomain) /** @todo remove */
                );
            }
