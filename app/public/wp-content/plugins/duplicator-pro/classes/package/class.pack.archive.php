...
use VendorDuplicator\ForceUTF8\Encoding;
...
//PHP 5 on windows decode patch
    ...
    private function addToDirList($dirPath, $relativePath, $size, $nodes)
    {
        ...
        if (strlen($relativePath) && !$global->skip_archive_scan) {
            ...
            $invalid_name = $invalid_encoding || strlen($dirPath) > PHP_MAXPATHLEN || strlen($trimmedName) === 0 || $name[strlen($name) - 1] === '.';
            if ($invalid_name) {
                if ($global->getBuildMode() != DUP_PRO_Archive_Build_Mode::Shell_Exec) {
...
//File Warnings
            ...
            $invalid_name     = $invalid_encoding || strlen($filePath) > PHP_MAXPATHLEN || strlen($trimmed_name) === 0;
            if ($invalid_encoding) {
