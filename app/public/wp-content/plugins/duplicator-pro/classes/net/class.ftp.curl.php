...
class DUP_PRO_FTPcURL
{
    ...
    public function create_directory($folder_path)
    {
        try {
            ...
            DUP_PRO_Log::trace("Error creating folder when using FTP cURL. FTP Info: " . $this->get_info() . ' Exception Error: ' . $e->getMessage());
            return false;
        }
        DUP_PRO_Log::trace("Successfully created folder by FTP cURL.");
        return true;
    }
     ...
     * @return bool
     */
    public function upload_file($local_file_path, $remote_file_path, $dest_filename = '')
    {
    ...
    public function download_file($remote_file_path, $local, $is_local_directory = true)
    {
        ...
        if (($file_stream = fopen($local_file_path, 'wb'))) {
            try {
                ...
                DUP_PRO_Log::trace("Error downloading " . $remote_file_path . " into " . $this->directory . ". FTP Info: " . $this->get_info() . ' Exception Error: ' . $e->getMessage());
                throw $e;
            }
     ...
     *
     * @return boolean|integer gives size of remote file or false on failure
     */
