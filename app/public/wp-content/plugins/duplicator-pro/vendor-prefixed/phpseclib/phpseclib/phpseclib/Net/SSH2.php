...
class SSH2
{
    ...
    function _key_exchange($kexinit_payload_server)
    {
        ...
        if (!\class_exists('VendorDuplicator\\Sodium')) {
        ...
        if ($encryption_algorithms === \false) {
            ...
            if (\class_exists('VendorDuplicator\\phpseclib\\Crypt\\RC4') === \false) {
                $encryption_algorithms = \array_diff($encryption_algorithms, array('arcfour256', 'arcfour128', 'arcfour'));
            }
            if (\class_exists('VendorDuplicator\\phpseclib\\Crypt\\Rijndael') === \false) {
            ...
            if (\class_exists('VendorDuplicator\\phpseclib\\Crypt\\Twofish') === \false) {
            ...
            if (\class_exists('VendorDuplicator\\phpseclib\\Crypt\\Blowfish') === \false) {
                $encryption_algorithms = \array_diff($encryption_algorithms, array('blowfish-ctr', 'blowfish-cbc'));
            }
            if (\class_exists('VendorDuplicator\\phpseclib\\Crypt\\TripleDES') === \false) {
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            $x = Random::string(32);
            $eBytes = \VendorDuplicator\Sodium::crypto_box_publickey_from_secretkey($x);
        ...
        if ($kex_algorithm === 'curve25519-sha256@libssh.org') {
            ...
            $key = new BigInteger(\VendorDuplicator\Sodium::crypto_scalarmult($x, $fBytes), 256);
            \VendorDuplicator\Sodium::sodium_memzero($x);
        } else {
        ...
        if ($this->bitmap & self::MASK_CONNECTED && $this->bitmap & self::MASK_LOGIN) {
            switch (\ord($payload[0])) {
                ...
                case NET_SSH2_MSG_CHANNEL_WINDOW_ADJUST:
                    ...
                    $payload = $this->bitmap & self::MASK_WINDOW_ADJUST ? \true : $this->_get_binary_packet();
            }
        }
        return $payload;
    }
