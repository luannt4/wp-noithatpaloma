...
function title_shortcode( $atts, $content = null ){
  ...
  $classes = array('section-title-container');
  ...
  return '<div class="' . esc_attr( $classes ) . '" ' . get_shortcode_inline_css($css_args) . '><'. $tag_name . ' class="section-title section-title-' . esc_attr( $style ) . '"'.get_shortcode_inline_css($css_args_title).'>' . $link_output . '<b aria-hidden="true"></b><span class="section-title-main" >' . $icon . wp_kses_post( $text ) . $small_text . '</span><b aria-hidden="true"></b></' . $tag_name . '></div>';
}
